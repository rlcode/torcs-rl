...
class Client(object):
    ...
    def setup_connection(self):
        ...
        try:
            ...
            print
            u'Error: Could not create socket...'
            sys.exit(-1)
        ...
        while True:
            ...
            try:
                ...
                print
                u"Waiting for server on %d............" % self.port
                print
                u"Count Down : " + unicode(n_fail)
                if n_fail < 0:
                    print
                    u"relaunch torcs"
                    ...
                    if self.vision is False:
                        ...
                        os.system(
                            u'torcs -nofuel -nodamage -nolaptime -vision &')
            ...
            if identify in sockdata:
                print
                u"Client connected on %d.............." % self.port
                break
        ...
        try:
            for opt in opts:
                ...
                if opt[0] == u'-v' or opt[0] == u'--version':
                    print
                    u'%s %s' % (sys.argv[0], version)
                    sys.exit(0)
        except ValueError, why:
            print
            u'Bad parameter \'%s\' for option %s: %s\n%s' % (
                opt[1], opt[0], why, usage)
            sys.exit(-1)
        if len(args) > 0:
            print
            u'Superflous input? %s\n%s' % (u', '.join(args), usage)
            sys.exit(-1)
    ...
    def get_servers_input(self):
        ...
        while True:
            ...
            if u'***identified***' in sockdata:
                ...
                # What do I do here?
                print
                u"Server has restarted the race on %d." % self.port
                ...
                if self.debug:
                    sys.stderr.write(
                        u"\x1b[2J\x1b[H")  # Clear for steady output.
                    print
                    self.S
    ...
    def respond_to_server(self):
        ...
        try:
            ...
            print
            u"Error sending to server: %s Message %s" % (
            emsg[1], unicode(emsg[0]))
            sys.exit(-1)
        if self.debug: print
        self.R.fancyout()
        ...
        for k in sensors:
            if type(self.d.get(k)) is list:  # Handle list type data.
                if k == u'track':  # Nice display for track sensors.
                    ...
                    strout += u' '.join(raw_tsens[:9]) + u'_' + raw_tsens[
                        9] + u'_' + u' '.join(raw_tsens[10:])
                    ...
                    for osensor in self.d[u'opponents']:
                        if osensor > 190:
                            ...
                            oc = unichr(int(osensor / 2) + 97 - 19)
                        elif osensor > 13:
                            oc = unichr(int(osensor) + 65 - 13)
                        elif osensor > 3:
                            oc = unichr(int(osensor) + 48 - 3)
                        else:
                            oc = u'?'
                        strout += oc
                ...
                if k == u'gear':  # This is redundant now since it's part of RPM.
                    ...
                    strout = u'%6.0f %s' % (
                        self.d[k], bargraph(self.d[k], 0, 10000, 50, u'~'))
                elif k == u'fuel':
                    strout = u'%6.0f %s' % (
                        self.d[k], bargraph(self.d[k], 0, 100, 50, u'f'))
                elif k == u'speedX':
                    cx = u'X'
                    if self.d[k] < 0: cx = u'R'
                    strout = u'%6.1f %s' % (
                        self.d[k], bargraph(self.d[k], -30, 300, 50, cx))
                elif k == u'speedY':  # This gets reversed for display to make sense.
                    strout = u'%6.1f %s' % (
                        self.d[k], bargraph(self.d[k] * -1, -25, 25, 50, u'Y'))
                elif k == u'speedZ':
                    strout = u'%6.1f %s' % (
                        self.d[k], bargraph(self.d[k], -13, 13, 50, u'Z'))
                elif k == u'z':
                    strout = u'%6.3f %s' % (
                        self.d[k], bargraph(self.d[k], .3, .5, 50, u'z'))
                    ...
                    if self.d[k]:
                        strout = u'%3d %s' % (
                            self.d[k], bargraph(self.d[k], 0, 300, 50, u"'"))
                    else:
                        strout = u'Not stuck!'
                elif k == u'rpm':
                    g = self.d[u'gear']
                    ...
                    if frontwheelradpersec:
                        skid = .5555555555 * self.d[
                            u'speedX'] / frontwheelradpersec - .66124
                    strout = bargraph(skid, -.05, .4, 50, u'*')
                elif k == u'slip':  # A sensible interpretation of wheel spin.
                    frontwheelradpersec = self.d[u'wheelSpinVel'][0]
                    slip = 0
                    if frontwheelradpersec:
                        slip = ((self.d[u'wheelSpinVel'][2] +
                                 self.d[u'wheelSpinVel'][3]) -
                                (self.d[u'wheelSpinVel'][0] +
                                 self.d[u'wheelSpinVel'][1]))
...
class DriverAction(object):
    ...
    def clip_to_limits(self):
        ...
        if self.d[u'meta'] not in [0, 1]:
            self.d[u'meta'] = 0
        if type(self.d[u'focus']) is not list or min(
                self.d[u'focus']) < -180 or max(self.d[u'focus']) > 180:
            self.d[u'focus'] = 0
    ...
    def fancyout(self):
        ...
        for k in sorted(od):
            if k == u'clutch' or k == u'brake' or k == u'accel':
                strout = u''
                strout = u'%6.3f %s' % (
                    od[k], bargraph(od[k], 0, 1, 50, k[0].upper()))
            elif k == u'steer':  # Reverse the graph to make sense.
                strout = u'%6.3f %s' % (
                    od[k], bargraph(od[k] * -1, -1, 1, 50, u'S'))
            else:
                strout = unicode(od[k])
            out += u"%s: %s\n" % (k, strout)
        return out
...
def destringify(s):
    ...
    if type(s) is unicode:
        try:
            ...
            print
            u"Could not find a value in %s" % s
            return s
    elif type(s) is list:
        if len(s) < 2:
